...
function updateDone() {
  document
    ...
    .getElementsByClassName('CTAT-done-button--text')[0].innerHTML = 'Submit';
}
  ...
  CTATCommShell.commShell.addGlobalEventListener({
    processCommShellEvent(event, message) {
      if (event == 'AssociatedRules' && message) {
        ...
        if (
          selectionElement &&
          selectionElement.classList.contains('CTATMathInput')
        )
  ...
  CTATCommShell.commShell.addGlobalEventListener({
    processCommShellEvent(event, message) {
      if (event == 'AssociatedRules' && message) {
        ...
        hintWindow.style.display = message.getProperty('TutorAdvice')
          ? 'flex'
          : 'none';
...
CTATExampleTracerSkill.prototype.clone = function () {
  let clone = new CTATExampleTracerSkill(
    this.getCategory(),
    this.getName(),
    this.getPGuess(),
    this.getPKnown(),
    ...
    processCommShellEvent(event, message) {
      if (event == 'StartProblem') {
        ...
        initialSkills = currentSkills
          ? currentSkills.getAllSkills().map((skill) => skill.clone())
          : [];
        ...
        let skillUpdates = message.getSkillsObject().getSkillSet(),
          skillLabelElements = document.getElementsByClassName(
            'CTATSkillWindow--label'
          );
        if (skillUpdates.length) {
          previousSkills.forEach((skill) => {
            let skillUpdate = skillUpdates.find(
                (skillUpdate) =>
                  `${skillUpdate.getSkillName()} ${skillUpdate.getCategory()}` ==
                  skill.getSkillName()
              ),
              skillLabelElement = Array.prototype.find.call(
                skillLabelElements,
                (skillLabelElement) =>
            ...
            if (skillUpdate) {
              let skillBarElement =
                skillLabelElement.previousSibling.firstElementChild;
              skillBarElement.style.width = skill.getPKnown() * 100 + '%';
              window.setTimeout(() => {
                skillBarElement.classList.add('transition');
                skillBarElement.style.width =
                  skillUpdate.getLevel() * 100 + '%';
              }, 0);
            } else skillLabelElement.parentNode.classList.add('blur');
          });
          window.setTimeout(() => {
            Array.prototype.forEach.call(
              skillLabelElements,
              (skillLabelElement) => {
                skillLabelElement.previousSibling.firstElementChild.classList.remove(
                  'transition'
                );
...
function setInitialSkills(skills) {
  let skillElementLabels = document.getElementsByClassName(
    'CTATSkillWindow--label'
  );
  skills.forEach((skill) => {
    ...
    Array.prototype.find
      .call(
        skillElementLabels,
        (labelElement) => labelElement.innerHTML == label
      )
